...
import { FiBell, FiSearch, FiChevronDown, FiMapPin, FiBarChart2, FiBookmark, FiArrowRight, FiMail, FiGlobe, FiShare2 } from "react-icons/fi";
...
function FilterPanel({ locations, setLocations, budgetRange, setBudgetRange, roiRange, setRoiRange, fundingStatus, setFundingStatus, onApply, onReset }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm">
      <div className="flex items-center justify-between mb-5">
        ...
        <button onClick={onReset} className="text-xs text-emerald-700 font-medium hover:underline">Reset Filters</button>
      </div>
      ...
      <div className="mb-5">
        <p className="text-xs font-semibold text-gray-700 mb-3 uppercase tracking-wider">Property Type</p>
        <div className="relative">
          ...
          <FiChevronDown className="absolute right-3 top-2.5 text-gray-400 pointer-events-none" size={15} />
        </div>
      </div>

      <div className="mb-5">
        <div className="flex items-center justify-between mb-2">
          <p className="text-xs font-semibold text-gray-700 uppercase tracking-wider">Budget Range</p>
          <span className="text-xs text-gray-500">$500k+</span>
        </div>
        <input type="range" min={100000} max={10000000} step={100000} value={budgetRange} onChange={e => setBudgetRange(Number(e.target.value))}
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          {FUNDING_STATUS.map(s => (
            <button key={s} onClick={() => toggleFunding(s)}
              className={`text-xs px-3 py-1.5 rounded-full font-medium border transition-colors ${fundingStatus === s ? "bg-emerald-700 text-white border-emerald-700" : "border-gray-200 text-gray-600 hover:border-emerald-300"}`}>
              {s}
            </button>
          ))}
        </div>
      ...
      <button onClick={onApply} className="w-full bg-emerald-700 hover:bg-emerald-800 text-white font-semibold py-2.5 rounded-xl text-sm transition-colors">
        Apply Filters
      </button>
    </div>
  );
...
function PropertyCard({ p }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow">
      <div className="relative">
        <img src={p.img} alt={p.name} className="w-full h-40 sm:h-44 object-cover" />
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-2 gap-3 mt-3">
          ...
          <div>
            ...
            <p className="text-sm font-bold text-gray-900 mt-1">{p.totalValue}</p>
          </div>
        </div>
        <div className="mt-3">
          <div className="flex items-center justify-between mb-1">
            <div>
              ...
              <p className="text-base font-extrabold text-gray-900">{p.sharePrice} <span className="text-xs text-gray-400 font-normal">/1%</span></p>
            </div>
            <span className="text-xs text-gray-500 font-medium">{p.funded}% Funded</span>
          </div>
          <div className="w-full bg-gray-100 rounded-full h-1.5">
            <div className="bg-emerald-700 h-1.5 rounded-full transition-all" style={{ width: `${p.funded}%` }}></div>
          </div>
        </div>
...
export default function PropertyPage() {
  ...
  const filtered = PROPERTIES.filter(p => {
    ...
    if (search && !p.name.toLowerCase().includes(search.toLowerCase()) && !p.loc.toLowerCase().includes(search.toLowerCase()) 
         ) return false;
     
    const matchedLocation = locations.find(l => 
      p.loc.toLowerCase().trim() === l.toLowerCase().trim()
    );
    ...
    <div className="min-h-screen bg-green-50 font-sans">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-8">
          <div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-gray-900 leading-tight">Explore Investment<br />Opportunities</h1>
            <p className="text-gray-500 mt-2 text-sm sm:text-base max-w-lg">Browse premium properties and invest in shares of high-yield real estate assets curated for institutional-grade portfolios.</p>
          </div>
          <div className="relative w-full sm:w-72 lg:w-80 shrink-0">
            <FiSearch className="absolute left-3 top-3 text-gray-400" size={16} />
            <input type="text" placeholder="Search by city, asset or type..." value={search} onChange={e => setSearch(e.target.value)}
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          <div className="w-full lg:w-56 xl:w-64 shrink-0">
            <FilterPanel
              ...
              roiRange={roiRange} setRoiRange={setRoiRange}
              fundingStatus={fundingStatus} setFundingStatus={setFundingStatus}
              onApply={() => {}} onReset={handleReset}
            />
          </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-5">
              <p className="text-sm text-gray-600">
                Showing <span className="font-semibold text-gray-900">{visibleProperties.length} properties</span> in Marketplace
              </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
              {visibleProperties.map(p => <PropertyCard key={p.name} p={p} />)}
            </div>
              ...
              <div className="flex items-center gap-2">
                {[1, 2, 3, "...", 8].map((p, i) => (
                  <button key={i} onClick={() => typeof p === "number" && setCurrentPage(p)}
                    className={`w-8 h-8 text-sm rounded-full font-medium transition-colors ${p === currentPage ? "bg-emerald-700 text-white" : "text-gray-500 hover:bg-gray-100"}`}>
                    {p}
                  </button>
                ))}
              </div>
